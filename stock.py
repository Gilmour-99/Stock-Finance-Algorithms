...
class stock():
    ...
    def MA(self):  # 设置k线图显示天数
        ...
        df3['upper_band'] = df3.close.rolling(
            20).mean()+stdev_factor*df3.close.rolling(20).std()
        df3['lower_band'] = df3.close.rolling(
            20).mean()-stdev_factor*df3.close.rolling(20).std()
    ...
    def k_plot(self):
        ...
        ax.text(df3.high.idxmax()-20, df3.high.max(),
                s=df3.high.max(), fontsize=8)
        ax.text(df3.high.idxmin()-20, df3.high.min(),
                s=df3.high.min(), fontsize=8)
        ...
        plt.plot(df3['upper_band'].values, alpha=0.5,
                 label='UPR', color='yellow')
    ...
    def forcast(self, time=1):
        '''
        可以对预测的期数进行调整
        预测期数time不能大于__n_periods__
        默认为 第一期 
        若大于__n_periods__则返回预测最末期均值
        '''
        period = self.__n_periods__
        time = int(time)-1
        if 0 <= time and time <= period:
            t = time
        elif time > period:
            t = period
            print('预测期数最大为%s' % period)
        else:
            t = 0
            print('预测期数需要为正值')
        df1 = self.MA()
    ...
    def ExpReturn1(self, time=1):
        '''
        使用时间序列方法计算期望收益率,只考虑收盘价
        '''
        fc_close = self.forcast(time)[1]
        forcast = fc_close
    ...
    def ExpReturn3(self, time=1):
        '''
        使用时间序列方法预测期望收益率（同时考虑开盘价和收盘价的波动）
        '''
        [fc_series_open, fc_series_close] = self.forcast(time)
...
def frontcon(ExpReturn, ExpCovariance, NumPorts=10):
    ...
    for tar in target_returns:
        ...
        res = sco.minimize(
            min_variance, noa * [1. / noa, ], method='SLSQP', bounds=bnds, constraints=cons)
